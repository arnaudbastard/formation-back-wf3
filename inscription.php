...
if (!empty($_POST)) {
    ...
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) { // Test de la validité de l'adresse email
    ...
    } elseif (strlen($_POST['cp']) !=5 || !ctype_digit ($_POST['cp'])) { // ctype_digit teste qu'une chaîne de caractères ne contient que des chiffres
    ...
    if (empty($mdp)) {
        ...
        $errors[] = 'Le mot de passe doit faire entre 6 et 20 caractères et ne doit contenir que des chiffres, des lettres ou les caractères _ et -';
    }
    ...
    if (!empty($errors)) { // EOS : end of string (attention très sensible aux espaces vides laissés avant et après, d'où le second EOS désindenté)
        $query = <<<EOS
