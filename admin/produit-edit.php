...
Si le formulaire est correctement rempli : INSERT en bdd et redirection vers la page de liste avec message de confirmation.
Sinon on affiche des messages d'erreurs et champs pré-remplis avec les valeurs saisies.

Adapter la page pour la modification.
- Si on reçoit un id dans l'URL sans retour de POST, on préremplit le formulaire à partir de la bdd
...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        if (!empty($_FILES['photo']['tmp_name'])) {
            ...
            // Le nom que va voir le fichier dans le répertoire photo
